...
<head>
    ...
    <title>Edit Stock Log - SantriKoding.com</title>
                    ...
                    <form action="{{ route('stocklogs.update', $stocklog->id) }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="product_id" class="form-control @error('product_id') is-invalid @enderror">
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $product->id == old('product_id', $stocklog->product_id) ? 'selected' : '' }}>
                                        {{ $product->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="change_type" class="form-control @error('change_type') is-invalid @enderror">
                                <option value="in" {{ old('change_type', $stocklog->change_type) == 'in' ? 'selected' : '' }}>IN (Masuk)</option>
                                <option value="out" {{ old('change_type', $stocklog->change_type) == 'out' ? 'selected' : '' }}>OUT (Keluar)</option>
                            </select>
                            @error('change_type')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" name="quantity" 
                                   value="{{ old('quantity', $stocklog->quantity) }}" 
